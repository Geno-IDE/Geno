...
bool _Win32HandleResult(DWORD result, std::string_view function,
  ...
  if (result != S_OK) {
    ...
    std::cerr << file << "(L" << line << ") " << function_name
              << " failed: " << buf << "\n";
...
bool _Win32HandleResult(HRESULT result, std::string_view function,
                        std::string_view file, int line) {
  if (result != S_OK) {
...
bool _Win32HandleResult(BOOL result, std::string_view function,
                        std::string_view file, int line) {
  if (result == FALSE) {
