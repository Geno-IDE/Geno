...
void TextEditWidget::Show(bool *p_open) {
    ...
    if (ImGuiWindow *window = ImGui::FindWindowByName(WINDOW_NAME)) {
      if (window->Rect().Contains(ImVec2(x, y))) {
        ...
        case Drop::TypeIndex::Bitmap: {
          ...
          dragged_bitmap_texture_.SetPixels(GL_RGBA8, bitmap.width,
                                            bitmap.height, GL_BGRA,
                                            bitmap.data.get());
          ...
          if (bitmap.width > bitmap.height)
            image_size =
                ImVec2(image_max_size,
                       image_max_size *
                           (bitmap.height / static_cast<float>(bitmap.width)));
          else
            image_size =
                ImVec2(image_max_size *
                           (bitmap.width / static_cast<float>(bitmap.height)),
                       image_max_size);

          ImGui::SetNextWindowPos(ImVec2(x, y));
          ImGui::BeginTooltip();
          ImGui::Image(dragged_bitmap_texture_.GetID(), image_size,
  ...
  if (ImGui::Begin(WINDOW_NAME, p_open)) {
    const int tab_bar_flags = (0 | ImGuiTabBarFlags_Reorderable |
                               ImGuiTabBarFlags_FittingPolicyScroll);

    if (!files_.empty() &&
        ImGui::BeginTabBar("TextEditTabBar", tab_bar_flags)) {
      for (File &file : files_) {
        ...
        if (ImGui::BeginTabItem(file_string.c_str(), &file.open)) {
          ...
          if (ImGui::InputTextMultiline("##TextEditor", &file.text,
                                        ImVec2(-0.01f, -0.01f),
                                        input_text_flags)) {
...
void TextEditWidget::AddFile(const std::filesystem::path &path) {
  if (!std::filesystem::exists(path)) {
    std::cerr << "Failed to add '" << path
              << "' to text-edit. File does not exist.\n";
    return;
  }
...
void TextEditWidget::OnDragDrop(const Drop &drop, int x, int y) {
  ...
  if (window && window->Rect().Contains(
