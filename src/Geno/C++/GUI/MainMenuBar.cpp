...
void MainMenuBar::Show(void) {
  ...
  if (ImGui::BeginMainMenuBar()) {
    ...
    if (ImGui::BeginMenu("File")) {
      ...
      if (ImGui::MenuItem("Close Workspace", "Ctrl+W"))
        ActionFileCloseWorkspace();

      ImGui::Separator();

      if (ImGui::MenuItem("Exit", "Alt+E"))
        ActionFileExit();
    ...
    if (ImGui::BeginMenu("View")) {
      if (ImGui::MenuItem("Text Edit", "Alt+T"))
        ActionViewTextEdit();
      if (ImGui::MenuItem("Workspace", "Alt+W"))
        ActionViewWorkspace();
      if (ImGui::MenuItem("Settings", "Alt+S"))
        ActionViewSettings();
      if (ImGui::MenuItem("Output", "Alt+O"))
        ActionViewOutput();
  ...
  if (ImGui::IsKeyDown(GLFW_KEY_LEFT_SHIFT) ||
    ...
    if (ImGui::IsKeyPressed(GLFW_KEY_W))
      ActionFileCloseWorkspace();
             ...
             ImGui::IsKeyDown(GLFW_KEY_RIGHT_ALT)) {
...
void MainMenuBar::ActionFileNewWorkspace(void) {
  NewItemModal::Instance().RequestPath(
      "New Workspace Location", std::filesystem::current_path(), this,
...
void MainMenuBar::ActionFileOpenWorkspace(void) {
  OpenFileModal::Instance().RequestFile(
      "Open Workspace", this,
...
void MainMenuBar::ActionFileExit(void) { exit(0); }
...
void MainMenuBar::ActionBuildBuildAndRun(void) {
  if (Workspace *workspace = Application::Instance().CurrentWorkspace()) {
    ...
    *workspace ^= [this](const WorkspaceBuildFinished &e) {
      ...
      std::cout << "=== " << output_string << " finished with exit code "
                << process.ExitCode() << " ===\n";
    };
...
void MainMenuBar::ActionViewTextEdit(void) { show_text_edit_ ^= 1; }

void MainMenuBar::ActionViewWorkspace(void) { show_workspace_ ^= 1; }
...
void MainMenuBar::ActionViewOutput(void) { show_output_ ^= 1; }

void MainMenuBar::ActionHelpDemo(void) { show_demo_window_ ^= 1; }

void MainMenuBar::ActionHelpAbout(void) { show_about_window_ ^= 1; }
