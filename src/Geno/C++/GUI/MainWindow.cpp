...
MainWindow::MainWindow(void) {
  ...
  if ((window_ = glfwCreateWindow(width_, height_, "Geno", nullptr, nullptr)) ==
      nullptr)
    return;

  glfwSetWindowUserPointer(window_, this);
  glfwMakeContextCurrent(window_);
  glfwSetWindowPos(window_, monitor.X() + (monitor.Width() - width_) / 2,
                   monitor.Y() + (monitor.Height() - height_) / 2);
...
void MainWindow::MakeCurrent(void) { glfwMakeContextCurrent(window_); }
...
void MainWindow::DragLeave(void) { dragged_drop_.reset(); }
